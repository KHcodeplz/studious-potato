...
<div class="row">
	...
	<button type="button" class="btn btn-outline-lightline-secondary" align="center">공지글
		작성</button>
</div>
<!-- 페이징 처리할 부분 -->
      <!-- <br><br><br><br><br> -->
      <div class="pagingArea" align="center">
         <button  onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=1'" class="btn btn-outline-light"><<</button>
         <%  if(currentPage <= 1) {  %>
         <button disabled class="btn btn-outline-light"><</button>
         <%  } else { %>
         <button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage - 1 %>'" class="btn btn-outline-light"><</button>
         <%  } %>
         ...
         <%      } else { %>
            <button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%= p %>'" class="btn btn-outline-light"><%= p %></button>
         <%      } %>
         <% } %>
         ...
         <button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage + 1 %>'" class="btn btn-outline-light">></button>
         <%  } %>
         <button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%= maxPage %>'" class="btn btn-outline-light">>></button>
      </div>
      <br><br>
